...
(function(){
  ...
  // Walls
  for(let x=-hw;x<=hw;x++){ t(x,30,'wall','South Wall') t(x,H,'wall','North Wall'); }
  for(let y=0;y<=H;y++){ t(-hw,y,'wall','West Wall'); t(hw,y,'wall','East Wall'); }

  // Gates
  t(0,30,'gate','South Gate',{exit:{layer:'overworld',pos:{x:400,y:440}}})
  t(0,H,'gate','North Gate',{exit:{layer:'overworld',pos:{x:400,y:438}}});
  t(-hw,0,'gate','West Gate',{exit:{layer:'overworld',pos:{x:399,y:439}}})
  t(hw,0,'gate','East Gate',{exit:{layer:'overworld',pos:{x:401,y:439}}})
