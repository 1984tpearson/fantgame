...
for(const{x:nx,y:ny,name}of npcPositions){
  ...
  if(cs>=24){ctx.fillStyle='rgba(136,136,128,0.85)';ctx.font=`${Math.max(8,cs*0.22)}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='top';ctx.fillText(name,sx2+cs/2,sy2+cs/2+r+1);}
}}

const leg=document.getElementById('map-legend');if(leg){leg.innerHTML='';lt.forEach(t=>leg.innerHTML+=`<div class="leg-item"><div class="leg-swatch t-${t==='road'||t==='river'?'plains':t}"></div>${terrainLabel(t)}</div>`);leg.innerHTML+=`<div class="leg-item"><div style="width:7px;height:7px;border-radius:50%;background:#888880;flex-shrink:0;"></div>NPC</div>`;}}
