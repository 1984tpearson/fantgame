...
function _getObjectCanvas(objId, seed, style, linen) {
  ...
  try {
    ...
    // Overworld settlement markers: settlement_city_0, settlement_town_2, etc.
      ...
      const stype = parts[1] || 'town';
      const variant = parseInt(parts[2] ?? style ?? 0) || 0;
      grid = MF.makeSettlementMarker(stype, variant, seed);
    }
  } catch(e) { return null; }
  if (!grid) return null;
  const c = document.createElement('canvas');
...
const ctx=canvas.getContext('2d');ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,W,H);const cs=CELL_PX*mapView.scale;const{x:px,y:py}=state.pos;const ox=W/2-(px*cs)+mapView.x,oy=H/2-(py*cs)+mapView.y;const x0=Math.floor(-ox/cs)-2,y0=Math.floor(-oy/cs)-2,x1=Math.floor((W-ox)/cs)+2,y1=Math.floor((H-oy)/cs)+2;const lt=new Set();const ss=seenSet();
...
if(cs>=10){
  ...
  if(obj==='_part'){
    ...
    if(tile){
      if(meta.object&&meta.object!=='_part'){
        // For settlement markers, look up dims by prefix
        const dimKey = meta.object.startsWith('settlement_')
          ? 'settlement_' + meta.object.split('_')[1]
          : meta.object;
        const dims=_OBJ_DIMS[dimKey];
        if(dims){
...
if(meta.doors&&meta.doors.length){ctx.globalAlpha=0.9;ctx.strokeStyle='#7a4a20';ctx.lineWidth=Math.max(2,cs*0.16);ctx.lineCap='round';const dm=cs*0.28,dc=cs/2;meta.doors.forEach(d=>{ctx.beginPath();if(d==='north'){ctx.moveTo(sx+dc-dm,sy+0.5);ctx.lineTo(sx+dc+dm,sy+0.5);}else if(d==='south'){ctx.moveTo(sx+dc-dm,sy+cs-1.5);ctx.lineTo(sx+dc+dm,sy+cs-1.5);}else if(d==='west'){ctx.moveTo(sx+0.5,sy+dc-dm);ctx.lineTo(sx+0.5,sy+dc+dm);}else if(d==='east'){ctx.moveTo(sx+cs-1.5,sy+dc-dm);ctx.lineTo(sx+cs-1.5,sy+dc+dm);}ctx.stroke();});ctx.globalAlpha=1;}
...
for(const[id,tmpl]of Object.entries(NPC_TEMPLATES)){
  ...
  for(const slot of(tmpl.schedule||[])){
    ...
    const parts=slot.posKey.split(',');
    if(parts.length===2){const nx=parseInt(parts[0]),ny=parseInt(parts[1]);if(!isNaN(nx)&&!isNaN(ny))npcPositions.push({x:nx,y:ny,name:tmpl.name,id,dynamic:!!tmpl.dynamic});}
    break;
  }
}
for(const[id,ns]of Object.entries(state.npcs)){
  ...
  if(parts.length===2){const nx=parseInt(parts[0]),ny=parseInt(parts[1]);if(!isNaN(nx)&&!isNaN(ny))npcPositions.push({x:nx,y:ny,name:NPC_TEMPLATES[id]?.name||'?',id,dynamic:!!NPC_TEMPLATES[id]?.dynamic});}
}
for(const np of npcPositions){const{x:nx,y:ny,name}=np;
  ...
  const _dc=np.dynamic?'#888880':'#4ab8f0';ctx.fillStyle=_dc;ctx.shadowColor=_dc;ctx.shadowBlur=r*1;ctx.fill();ctx.shadowBlur=0;
  if(cs>=24){ctx.fillStyle=np.dynamic?'rgba(136,136,128,0.85)':'rgba(74,184,240,0.85)';ctx.font=`${Math.max(8,cs*0.22)}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='top';ctx.fillText(name,sx2+cs/2,sy2+cs/2+r+1);}
}}

const leg=document.getElementById('map-legend');if(leg){leg.innerHTML='';lt.forEach(t=>leg.innerHTML+=`<div class="leg-item"><div class="leg-swatch t-${t==='road'||t==='river'?'plains':t}"></div>${terrainLabel(t)}</div>`);leg.innerHTML+=`<div class="leg-item"><div style="width:7px;height:7px;border-radius:50%;background:#4ab8f0;flex-shrink:0;"></div>NPC</div><div class="leg-item"><div style="width:7px;height:7px;border-radius:50%;background:#888880;flex-shrink:0;"></div>Encountered</div>`;}}
...
canvas.addEventListener('mousemove',e=>{
  ...
  const rect=canvas.getBoundingClientRect();const W=canvas.width,H=canvas.height;
  const scaleX=W/rect.width,scaleY=H/rect.height;
  const mcx=(e.clientX-rect.left)*scaleX,mcy=(e.clientY-rect.top)*scaleY;
  const cs=CELL_PX*mapView.scale;const{x:px,y:py}=state.pos;
  const ox=W/2-(px*cs)+mapView.x,oy=H/2-(py*cs)+mapView.y;
  const hx=Math.floor((mcx-ox)/cs),hy=Math.floor((mcy-oy)/cs);
  const hmeta=getVisibleCellMeta(hx,hy);
  let tip=document.getElementById('map-tooltip');
  if(!tip){tip=document.createElement('div');tip.id='map-tooltip';tip.style.cssText='position:fixed;background:rgba(10,8,6,0.85);border:1px solid rgba(201,148,58,0.4);color:#e8c87a;font-size:0.72rem;font-family:"Crimson Pro",serif;padding:3px 8px;border-radius:2px;pointer-events:none;z-index:9999;display:none;';document.body.appendChild(tip);}
  if(hmeta.name&&(hmeta.type===T.BUILDING||hmeta.type===T.MARKET||hmeta.type===T.DOCKS)){
    tip.textContent=hmeta.name;tip.style.display='block';
    tip.style.left=(e.clientX+14)+'px';tip.style.top=(e.clientY-8)+'px';
  } else {tip.style.display='none';}
});canvas.addEventListener('wheel',e=>{e.preventDefault();const d=e.deltaY>0?0.9:1.1;mapView.scale=Math.max(0.08,Math.min(4,mapView.scale*d));drawMapCanvas();},{passive:false});}
...
// Build NPC cell set for dot rendering
const npcCells=new Set();
const _hour=(state.player.day%1)*24;
for(const[id,tmpl]of Object.entries(NPC_TEMPLATES)){
  if(tmpl.dynamic)continue;
  for(const slot of(tmpl.schedule||[])){
    if(slot.layer!==state.layer)continue;
    if(slot.settlementId&&slot.settlementId!==state.settlementId)continue;
    const _active=slot.timeStart<slot.timeEnd?(_hour>=slot.timeStart&&_hour<slot.timeEnd):(_hour>=slot.timeStart||_hour<slot.timeEnd);
    if(!_active)continue;
...
// ═══════════════════════════════════════════════════
function layerContext(){if(state.layer==='interior')return'INTERIOR SCALE: 2m per cell. Inside a building. Describe room details: furniture, light, smells, sounds, occupants.';if(state.layer==='settlement'){const name=SETTLEMENTS[state.settlementId]?.name||state.settlementId;const s=SETTLEMENTS[state.settlementId];const hasWalls=s?.hasWalls!==false;const gateInfo=s?Object.entries(s.map).filter(([k,v])=>v.type==='gate').map(([k,v])=>{const[gx,gy]=k.split(',').map(Number);return`${v.name} at local (${gx},${gy})`;}).join(', '):'';const exitNote=hasWalls?`GATES: ${gateInfo}. IMPORTANT: if player says they leave via a specific gate (e.g. north gate), set exitVia to that gate name in JSON so the engine uses the correct exit. exitVia options match gate names above.`:`EXITS: This settlement has no walls or gates. Players leave by walking to the edge of town along any road. Do NOT mention gates or city walls — they do not exist here. Set exitVia to the road direction (e.g. 'north road') when the player leaves.`;return`SETTLEMENT SCALE: 20m per cell. Inside ${name}. Describe street-scale details: stalls, doorways, crowds, signage, cobblestones.\n${exitNote}`;}return'OVERWORLD SCALE: ~280m per cell across a large island kingdom. Describe landscape, weather, distant landmarks, feel of the region.';}
...
function spawnNpc(spawnData, cellKeyStr) {
  // Check if an NPC with this name already exists — don't duplicate
  const existingId = Object.keys(NPC_TEMPLATES).find(id => 
    NPC_TEMPLATES[id].name.toLowerCase() === spawnData.name.toLowerCase()
  );
  if (existingId) {
    // Update their cellKey to current position
    if (state.npcs[existingId]) state.npcs[existingId].cellKey = cellKeyStr;
    return existingId;
  }
