...
if(meta.type===T.BUILDING&&meta.doors&&meta.doors.length){ctx.globalAlpha=0.9;ctx.strokeStyle='#e8b84b';ctx.lineWidth=Math.max(2,cs*0.14);ctx.lineCap='round';const dm=cs*0.28,dc=cs/2;meta.doors.forEach(d=>{ctx.beginPath();if(d==='north'){ctx.moveTo(sx+dc-dm,sy+0.5);ctx.lineTo(sx+dc+dm,sy+0.5);}else if(d==='south'){ctx.moveTo(sx+dc-dm,sy+cs-1.5);ctx.lineTo(sx+dc+dm,sy+cs-1.5);}else if(d==='west'){ctx.moveTo(sx+0.5,sy+dc-dm);ctx.lineTo(sx+0.5,sy+dc+dm);}else if(d==='east'){ctx.moveTo(sx+cs-1.5,sy+dc-dm);ctx.lineTo(sx+cs-1.5,sy+dc+dm);}ctx.stroke();});ctx.globalAlpha=1;}
